...
//const GLchar* shader_color_first = R"(#version 330 core
//out vec4 color;
//void main()
//{
...
//
//// shader_color - это встроенная переменная, которая определяет цвет вершин в пространстве, это результат работы шейдера
//const GLchar* shader_color_second = R"(#version 330 core
//out vec4 color;
//void main()
//{
//	//color = vec4(1.0f, 0.5f, 0.2f, 1.0f);
...
//
//	// Шейдер2
//	GLuint fragmentShader_red = glCreateShader(GL_FRAGMENT_SHADER);
//	glShaderSource(fragmentShader_red, 1, &shader_color_second, NULL);
//	glCompileShader(fragmentShader_red);
//	// Ссылка на шейдеры, к ней привязываются шейдеры
//	GLuint shaderProgram_red = glCreateProgram();
